...
class AudioTransformerSingleLinearReconstruction(nn.Module):
    def __init__(self, input_dim=64, num_heads=16, transformer_layers=8, length=256, d_model=256, dim_feedforward=512, dropout=0.1,
                 latent_space=64):
        super(AudioTransformerSingleLinearReconstruction, self).__init__()
        self.name = f"AudioTransformerSingleLinearReconstruction-LatentSpace{latent_space}-Heads{num_heads}-TrasformerLayers{transformer_layers}-DModel{length}-Dropout{dropout}"
